#!/usr/bin/env python3
"""
🌊 Demo Rápido - Visualización Oceanográfica
Ejemplo que funciona solo con bibliotecas básicas (numpy, matplotlib)
"""
...
def create_bathymetry_simple():
    ...
    ax1.set_xlabel('X (km)')
    ax1.set_ylabel('Y (km)')
    ax1.set_title('🏔️ Monte Submarino - Vista en Planta')
...
def create_ocean_currents():
    ...
    # Líneas de corriente
    ax.streamplot(X, Y, U, V, color='white', density=1.5, linewidth=1.5, alpha=0.7)
...
def create_hydrographic_profile():
    ...
    axes[0].set_title('🌡️ Temperatura')
...
def create_time_series():
    ...
    axes[0].set_title('🌡️ Temperatura Superficial del Mar')
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()
...
def main():
    ...
    print("🌊" + "="*60)
    print("     DEMO RÁPIDO - VISUALIZACIONES OCEANOGRÁFICAS")
    print("="*63)
    ...
    try:
        ...
        print(f"❌ Error: {e}")
        print("🔧 Asegúrate de tener instalados: numpy, matplotlib")
